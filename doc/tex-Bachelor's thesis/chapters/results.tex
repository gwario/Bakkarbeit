...
The following graphs are not calculated following exact formulas, although the proportions are realistic. In [fig1] I used circles to to represent single words. The radii and text size reflect the number of executions within the time frame, small radii mean fewer executions. The position reflects the number of words executed within a word, the root-word of the trace snapshot is in the center of the graph, fewer sub-word-executions result in a greater distance from the root word.
In [fig1], the execution of \emph{m} is chosen as the root word, the trace contains m and the following 201 executed words.
