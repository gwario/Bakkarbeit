...
It should cover \emph{value}, \emph{variable}, \emph{2variable}, \emph{fvariable} and also memory fields, allocated by custom defining words. \ref{fig:taxonomy} shows only a small snapshot of the whole graph, because the whole graph would have been too large(the graph grows with the number of words, which access memory locations).
...
\\
Gfvis is started by executing \verb|<path to modified gforth>/gforth-itc ./gfvis.fs|. When the dbg is started(i.e.\ \verb|dbg test|), trace.ps, a copy of gfvis.ps, is created and gv is started to display its content(See \ref{fig:traceps1} and \ref{fig:traceps2}). From then on, every executed word, updates trace.ps with the name of the word and the state of the stacks. When \verb|bye| is executed, gfvis terminates gv and gforth-itc.

\subsubsection*{Interpretation}

In \ref{fig:traceps1} and \ref{fig:traceps2}, I used the word \emph{test} to demonstrate the use of gfvis. Test contains conditionals, numbers, mathematical operations, sub word executions and it prints text to stdout. The source code of \emph{test} can be found in \ref{fig:gfvis-test-src}. With gfvis it is easy to follow the inner working of \emph{test}.

\subsubsection*{Possible improvements}

The first problem of gfvis is the very inefficient use of display space. It would be better to show only the current and the previous stack state. The complete history should be shown only on demand. Also the displayed stack depth could be limited to a few words(e. g. the top 5 elements).
...
Another improvement would be to show not only the stack state, but also the content of value, variable and custom defined memory.

Displaying all allocated memory areas and keep track of changes within those areas in a \emph{diff} like manner,  would be helpful.

Better visualization of beginning and end of control structures and especially loops would also be an improvement.
